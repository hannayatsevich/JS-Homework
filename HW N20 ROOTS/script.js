...
function squareRootsTests() {
  ...
  console.log('тест 2,0,0 -> один корень 0');
  var roots=squareRoots(2, 0, 0);
  console.log( ((roots.length == 1) && (roots[0] == 0))?'пройден':'НЕ ПРОЙДЕН!' );

  console.log('тест 4,0,36 -> нет корней');
  var roots=squareRoots(4, 0, 36);
  console.log( (roots.length == 0)?'пройден':'НЕ ПРОЙДЕН!' );

  console.log('тест 4,-7,0 -> два корня 1.75,0');
  var roots=squareRoots(4, -7, 0);
  console.log( ((roots.length == 2) && (roots[0] == 1.75) && (roots[1] == 0))?'пройден':'НЕ ПРОЙДЕН!' );

  console.log('тест 0,0,5 -> нет корней');
  var roots = squareRoots(0, 0, 5);
  console.log( (roots.length == 0)?'пройден':'НЕ ПРОЙДЕН!' );
};

function squareRootsInput() {
